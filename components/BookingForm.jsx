...
const BookingForm = ({ room }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Exécuter l'action si la validation est correcte
  ...
  return (
    ...
    <form className="mt-4" onSubmit={handleSubmit}>
      ...
      <div className="mt-6">
        <button
          type="submit"
          className="flex justify-center w-full px-4 py-3 text-sm font-medium text-white bg-black border border-transparent rounded-md shadow-sm hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 disabled:opacity-50"
          disabled={isLoading}
        >
          {isLoading ? 'Réservation en cours...' : 'Réserver la salle'}
        </button>
      </div>
    </form>
  </div>
